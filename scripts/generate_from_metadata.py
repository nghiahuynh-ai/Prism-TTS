...
class PrismTTSBatchGenerator:
    """Load Prism-TTS/Mimi once, then synthesize metadata entries sequentially."""
    ...
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.two_stage = has_discrete and has_continuous
        if self.two_stage:
            self.discrete_model = generate_utils.build_model(
                generate_utils.read_yaml(args.discrete_model_config)
            )
            self.continuous_model = generate_utils.build_model(
                generate_utils.read_yaml(args.continuous_model_config)
            )
            if getattr(self.discrete_model, "stage", None) != "discrete":
                raise ValueError("--discrete-model-config must select model.name=prism_discrete.")
            if getattr(self.continuous_model, "stage", None) != "continuous_meanflow":
                raise ValueError(
                    "--continuous-model-config must select model.name=prism_continuous_meanflow."
                )
            if (
                self.discrete_model.num_discrete_tokens
                != self.continuous_model.num_discrete_tokens
                or self.discrete_model.discrete_vocab_size
                != self.continuous_model.discrete_vocab_size
            ):
                raise ValueError("Discrete and continuous stage configs have incompatible token layouts.")
            generate_utils.load_checkpoint(
                self.discrete_model, args.discrete_checkpoint, use_ema=bool(args.use_ema)
            )
            generate_utils.load_checkpoint(
                self.continuous_model, args.continuous_checkpoint, use_ema=bool(args.use_ema)
            )
            self.discrete_model.to(device=self.device, dtype=self.model_dtype).eval()
            self.continuous_model.to(device=self.device, dtype=self.model_dtype).eval()
            self.model = self.continuous_model
        else:
            if args.checkpoint is None:
                raise ValueError(
                    "Provide --checkpoint for legacy joint generation, or both stage-specific checkpoints."
                )
            self.model = generate_utils.build_model(generate_utils.read_yaml(args.model_config))
            if getattr(self.model, "stage", None) is not None:
            ...
            self.model.to(device=self.device, dtype=self.model_dtype).eval()
            self.discrete_model = self.model
            self.continuous_model = self.model
        ...
        self.continuous_window_size = _parse_continuous_window_size(args.continuous_window_size)
    ...
    def _encode_prompt(self, prompt_audio_path: Path) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        raw_discrete = prompt_codes[0, : self.discrete_model.num_discrete_tokens].transpose(0, 1).to(
            dtype=torch.long
        ).cpu()
    ...
    def _run_generation(
        ...
        *,
        ...
        with torch.inference_mode():
            if self.two_stage:
                discrete_generation = self.discrete_model.generate(
                ...
                generation = self.continuous_model.generate(
                    text_prompt=text_prompt,
                    discrete_prompt=discrete_prompt,
                    ...
                    discrete_target_lengths=torch.tensor(
                        [discrete_generation.discrete_ids.shape[-1]],
                        device=self.device,
                        dtype=torch.long,
                    ),
                    window_size=self.continuous_window_size,
                    flow_num_steps=self.args.flow_num_steps,
                    time_stagger=self.args.continuous_time_stagger,
                    return_dict=True,
                )
        ...
        *,
        ...
        def summarize_attempt(label: str, candidate: Any) -> dict[str, float] | None:
            ...
            stats = generate_utils.summarize_discrete_generation(
                ...
                num_discrete_tokens=int(self.discrete_model.num_discrete_tokens),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Legacy joint PrismTTS checkpoint (.ckpt/.pt).",
    )
    ...
    parser.add_argument(
        ...
        help="Stage-1 discrete model config.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Stage-2 continuous MeanFlow model config.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="1",
        help="One positive patch width or comma-separated widths for two-stage continuous generation.",
    )
    parser.add_argument(
